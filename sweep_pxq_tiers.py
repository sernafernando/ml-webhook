#!/usr/bin/env python3
"""Trigger /admin/sweep-pxq-tiers and poll status until done. Cron-friendly.

Clone of sweep_shipping_costs.py — same flags, same polling, same exit codes.
Only stdlib: no venv, no requirements, nothing to activate from cron.
"""
import argparse
import json
import os
...
def http_get(url, timeout=30):
    req = Request(url, headers={"User-Agent": "sweep-pxq-tiers/1.0"})
    with urlopen(req, timeout=timeout) as r:
        return r.status, json.loads(r.read().decode("utf-8"))


def main():
    ap = argparse.ArgumentParser(description="Trigger and watch the PxQ price-tier sweep.")
    ...
    ap.add_argument("--dry-run", action="store_true", help="enumerate only, no writes")
    ...
    start_url = f"{args.base}/admin/sweep-pxq-tiers"
    if params:
    ...
    poll_url = f"{args.base}/admin/sweep-pxq-tiers?status=1"
    ...
    while time.time() < deadline:
        ...
        print(
            ...
            f"with_tiers={state.get('with_tiers')} "
